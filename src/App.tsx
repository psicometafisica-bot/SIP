    ...
    const loadInventory = async () => {
      try {
        ...
        if (!Array.isArray(initialInventory)) {
          throw new Error("La respuesta de la IA para el inventario no es una lista válida.");
        }
        setInventory(initialInventory);
        setError(null);
      } catch (err) {
        console.error(err);
        setError(`Error fatal al cargar el inventario inicial. La simulación no puede continuar. (${err instanceof Error ? err.message : String(err)})`);
      } finally {
        setLoading(false);
      }
    };
